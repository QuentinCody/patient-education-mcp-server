...
export function registerQueryData(server: McpServer, env?: QueryEnv) {
    ...
    server.registerTool(
        "pated_query_data",
        {
            title: "Query Staged Patient Education Data",
            description:
                "Query staged patient education data using SQL. Use this when responses are too large and have been staged with a data_access_id.",
            inputSchema: {
                data_access_id: z
                    .string()
                    .min(1)
